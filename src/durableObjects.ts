...
import { sign, createMessage, createCleartextMessage, readPrivateKey, PrivateKey } from 'openpgp';
...
function buildPackagesFile(rows: { metadata: string }[]): string {
	...
	return rows.map((r) => formatPackagesEntry(JSON.parse(r.metadata) as Record<string, unknown>)).join('\n\n') + '\n';
}

function buildArchReleaseFile(component: string, architecture: string, suite: string, origin: string, label: string): string {
	return (
		[
			`Archive: ${suite}`,
			`Suite: ${suite}`,
...
distApp.get('/InRelease', async (c) => {
	const inRelease = await c.env.loadInRelease();
...
export class DistDurableObject extends DurableObject<Env> {
	...
	public inRelease: string | undefined;
	...
	async loadInRelease(): Promise<string> {
		if (this.inRelease) return this.inRelease;
		const { release } = await this.loadIndices();
		const privateKey = (this.gpgPrivateKey ??= await readPrivateKey({ armoredKey: this.env.GPG_PRIVATE_KEY }));
		const cleartextMessage = await createCleartextMessage({ text: release });
		const signed = await sign({
			message: cleartextMessage,
			signingKeys: [privateKey],
		});
		this.inRelease = signed as string;
	...
	private async buildIndices(): Promise<{ release: string; artifacts: Map<string, Uint8Array> }> {
		...
		for (const { cat_name, architecture } of pairs) {
			...
			const rows = sql
				.exec<{
					metadata: string;
				}>('SELECT metadata FROM releases WHERE cat_name = ? AND architecture = ? ORDER BY id', cat_name, architecture)
				.toArray();

			const packagesBuf = new TextEncoder().encode(buildPackagesFile(rows));
			const packagesGz = await gzipBytes(packagesBuf);
			const archReleaseBuf = new TextEncoder().encode(buildArchReleaseFile(cat_name, architecture, distName, origin, label));
