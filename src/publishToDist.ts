...
export async function publishToDist(c: Context<{ Bindings: DistDurableObject }>): Promise<Response> {
    ...
    for (const deb of debs) {
        ...
        const file = await c.env.env.POOL_BUCKET.get(
            `${encodeURIComponent(repoName)}/${encodeURIComponent(fileId)}`,
        );
        if (!file) {
    ...
    c.env.ctx.storage.transactionSync(() => {
        for (const controlField of published) {
            ...
            c.env.ctx.storage.sql.exec('INSERT INTO releases (cat_name, id, metadata, architecture) VALUES (?, ?, ?, ?) ON CONFLICT(cat_name, id) DO UPDATE SET metadata = ?', catName, fileId, j, cpy.Architecture ?? 'all', j);
        }
    });
